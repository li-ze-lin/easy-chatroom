 ...
 * 存储信息
 */
public class MultiplayerChatroomInformation {

    public static ConcurrentMap<String, ConcurrentMap<String, MultiplayerChatroomInformation>> map = new ConcurrentHashMap<>();

    private ChannelHandlerContext ctx;
    private Mage mage;

    private MultiplayerChatroomInformation(ChannelHandlerContext ctx, Mage mage) {
        this.ctx = ctx;
        this.mage = mage;
    }
    ...
    public static void add(ChannelHandlerContext ctx, Mage mage) {
        MultiplayerChatroomInformation iom = new MultiplayerChatroomInformation(ctx, mage);
        ConcurrentMap<String, MultiplayerChatroomInformation> cmap = new ConcurrentHashMap<>();
    ...
    public static void delete(String id, String table) {
        ConcurrentMap<String, MultiplayerChatroomInformation> cmap = map.get(table);
        if (cmap.size() <= 1) {
            map.remove(table);
        } else {
            cmap.remove(id);
